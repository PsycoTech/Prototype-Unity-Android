...
public class grid_map : MonoBehaviour
{
    ...
    // - interact
    // - mob
    public static grid_map Instance;
    // [static]
    ...
    void Start()
    {
        ...
        for (int x = 0; x < _sizeGrid_Int.x; x++)
            for (int y = 0; y < _sizeGrid_Int.y; y++)
                _gridStatic[x, y] = Physics2D.OverlapCircle(IndexToWorld(x, y), .1f, game_variables.Instance.ScanLayerSolid) == null;
        // grid [dynamic]
    ...
    void Update()
    {
        ...
        {
            _timer = _time;
            List<Vector2Int[]> chunks = manager_chunk.Instance.GetActive();
            if (chunks.Count == 0)
                return;
